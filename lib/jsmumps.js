...
JSMumps.prototype.routine = function(routine, async) {
    ...
    for(var i in directories) {
        this.logger.info("routine(): scanning " + directories[i] + " for " + routine);
        file = directories[i] + "/" + routine + ".m";

        if(fs.existsSync(file)) {
            this.logger.info("routine(): found routine " + routine + " at " + file);
            break;
        }

    }

    this.logger.info("routine(): parsing " + file);

    var fd = fs.openSync(file, "r");
    var lines = fs.readFileSync(fd, "utf-8").split("\n");

    this.logger.info("routine(): read " + lines.length + " lines from " + file);
    ...
    for(line in lines) {
        if(lines[line].match(/^[%A-Za-z0-9]+/)) {
            ...
            this.logger.info("routine(): found label " + label + " at line " + (line + 1));
