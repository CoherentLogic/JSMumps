...
function Server(options)
{
    ...
    Server.prototype.CreateSession = function(cookies) {
	crypto = require('crypto');

	var sessionKey = crypto.randomBytes(48).toString('hex');

	that.DB.set({global: 'JSM', subscripts: ['sessions', sessionKey], data: ''});
	cookies.set('JSMSESSID', sessionKey, {maxAge: that.MaxSessionAge});
	return sessionKey
    }
	...
	var sessionID = cookies.get('JSMSESSID');
	if(!sessionID) {	    
	    sessionID = that.CreateSession(cookies);
	    that.WriteLog("NEW session ID is " + sessionID);
	}
	else {
	    that.WriteLog("EXISTING session ID is " + sessionID);
	}
