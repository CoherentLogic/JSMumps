...
function Server(options)
{
	...
	if(that.Cluster.isMaster) {
	    ...
	    process.on('SIGINT', function(err, sig) {
		that.WriteLog("Server shutting down");
		that.DB.close();
		process.exit(1);
	    });

	    that.WriteLog("Worker process startup beginning");

	    that.http = require('http');
	    that.server = that.http.createServer(that.OnRequest);

	    that.WriteLog("Attaching to TCP port " + that.Port);
	    that.server.listen(that.Port, that.OnListen);

	    that.WriteLog("Initializing MUMPS database connectivity");

	    var dbResult = that.DB.open();
	    if(dbResult.ok === 1) {
		that.WriteLog("MUMPS database connected successfully (" + that.DB.version() + ")");
		for(i = 0; i < 100; i++) {
		    that.DB.get({global: "JPW", subscripts: ["smsTest", 48, 1]}, function(err, res) {
		    if(err) {
			that.WriteLog("Get error message is " + JSON.stringify(err));
		    }
		    else {
			that.WriteLog("Get result is " + JSON.stringify(res));			
		    }
		});		
		    that.DB.set("JPW", "smsTest", 48, 1, process.pid, function(err, res) {
		    if(err) {
			that.WriteLog("Set error message is " + JSON.stringify(err));
		    }
		    else {
			that.WriteLog("Set result is " + JSON.stringify(res));			
		    }
		});		
		}
	    ...
	    that.WriteLog("Server startup complete.");
	}
    };   
